...
namespace HZ.IDTSCore.Api
{
    ...
    public class DeviceThread: IHostedService
    {
        ...
        private void PushThreadHandle()
        {
            while (true)
            {
                if (deviceObjcetList.Count > 0)
                {
                    ...
                    // 2026-06-10 优化：推送线程使用实时采集快照，避免接口线程刷新采集集合时枚举异常。
                    GlobalRealCollectViewModel realCollectSnapshot = DeviceDriver.Instance.GetRealCollectSnapshotV2();
                    foreach (var device in deviceObjcetList)
                    {
                        var cacheDevice = DeviceDriver.Instance.GetDevice(device.cn_s_equi_no);
                        if (cacheDevice != null) device.cn_s_equi_status = cacheDevice.deviceState;

                        if (realCollectSnapshot.eqRealCollect.FirstOrDefault(p => p.deviceNo == device.cn_s_equi_no) == null)
                        {
                    ...
                    {
                        _deviceRealCollect = new GlobalRealCollectViewModel();
                        _deviceRealCollect.eqRealCollect = realCollectSnapshot.eqRealCollect.Where(p => p.deviceNo == DeviceDriver.Instance.DExplosion.deviceCode).ToList();
                        //推送设备JSON数据(设备采集事项)
                        sendJSONString = JsonConvert.SerializeObject(_deviceRealCollect);
                    }
                    else
                    {
                        sendJSONString = JsonConvert.SerializeObject(realCollectSnapshot);
                    }
        ...
        public void ErrorsPushThreadHandle()
        {
            ...
            while (true)
            {
                List<GlobalDevicesViewsModel> stateSnapshot = DeviceDriver.Instance.GetStateListSnapshotV2();
                if (stateSnapshot.Count > 0)
                {
                    AlarmCollectList.eqAlarmCollect.Clear();
                    // 2026-06-10 优化：遍历设备状态快照，避免接口线程修改StateList时触发Collection was modified。
                    foreach (var driver in stateSnapshot)
                    {
