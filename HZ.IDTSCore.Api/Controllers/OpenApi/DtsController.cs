...
using HZ.IDTSCore.Interfaces.Service.Common;
...
namespace HZ.IDTSCore.Api.Controllers.OpenApi
{
    ...
    public class DtsController : BaseController
    {
        ...
        /// </summary>
        private static string BuildLocationStateLogSummary(List<LocationStateWMS> locationStateInfoList, string rawLocationJson)
        {
            int totalCount = locationStateInfoList == null ? 0 : locationStateInfoList.Count;
            int moveLocationCount = locationStateInfoList == null ? 0 : locationStateInfoList.Count(it => it.moveLocations != null);
            int containerCount = locationStateInfoList == null ? 0 : locationStateInfoList.Sum(it => it.moveLocations == null || it.moveLocations.containers == null ? 0 : it.moveLocations.containers.Count);
            string sampleCodes = locationStateInfoList == null ? "" : string.Join(",", locationStateInfoList.Take(LargeJsonLogSampleCount).Select(it => it.stockCode + "/" + it.areaCode + "/" + it.fixLocationCode));
            return "WMS货位状态返回摘要：总数=" + totalCount + "，有移动货位数=" + moveLocationCount + "，料箱数=" + containerCount + "，原始响应长度=" + (rawLocationJson == null ? 0 : rawLocationJson.Length) + "，样例=" + sampleCodes;
        }

        /// <summary>
        /// 大集合日志只记录统计信息和少量样例，避免序列化完整 LocationRealMonitorViewModel。
        /// </summary>
        private static string BuildLocationRealMonitorLogSummary(LocationRealMonitorViewModel model)
        {
            var stocks = model == null ? null : model.stock;
            ...
            if (user != null)
            {
                ...
                if (!(locationApiSetting is null))
                {
                    ...
                    try
                    {
                        ...
                        LogHelper.Info(BuildLocationStateLogSummary(locationStateInfoList, strLocationList));
                ...
                try
                {
                    LogHelper.Info(BuildLocationRealMonitorLogSummary(locationRealMonitorViewModel));
        ...
        public IActionResult GoodsQuerylist(string goodsCode, string goodsName, string pileofland = "")
        {
            ...
            if (pileofland == "DD691")
            {
                try
                {
                    string itemCodeName = string.IsNullOrEmpty(goodsCode) == true ? "" : goodsCode;
                    itemCodeName = string.IsNullOrEmpty(goodsName) == true ? "" : goodsName;
                    ...
                    string areaCode = _ISettingService.GetFirst(it => it.cn_s_setting_keycode == "WMSLocationApiAreaCode").cn_s_setting_keyvalue;
                    ApiResult apiResLocation = new ApiResult();
                    string strGoodsList = WebApiManager.HttpGet(baseUrl, pathAndQuery + $"?stockCode={stockCode}&areaCode={areaCode}&itemCodeName={itemCodeName}", ref apiResLocation);
        ...
        public async Task<IActionResult> syncFullLocation()
        {
            ...
            if (user != null)
            {
                tn_dts_setting trayApiSetting = _ISettingService.GetFirst(it => it.cn_s_setting_keycode == "GetAllLocationTwinState");
                if (!(trayApiSetting is null))
                {
                    string trayApi = trayApiSetting.cn_s_setting_keyvalue;
                    try
                    {
                        ...
                        ApiResult apiResLocation = new ApiResult();
                        string query = "";
                        string resultString = WebApiManager.HttpPost(baseUrl, pathAndQuery + query, ref apiResLocation);
                        if (resultString != "")
                        {
                            LocationRealMonitorViewModel model = JsonConvert.DeserializeObject<LocationRealMonitorViewModel>(resultString);
                            if (model != null)
                            {
                                //从Redis获取全量的货位数据
                                await LocationRealMonitorDriver.Instance.LocationRealMonitorProc(model);
                ...
                else
                {
                    result.IsSuccess = false;
                    result.ErrCode = 500;
                    result.Message = "缺少【GetAllLocationTwinState】系统配置项";
                }
